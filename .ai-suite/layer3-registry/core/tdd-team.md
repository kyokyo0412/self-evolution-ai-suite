...
- **3.4 REFACTOR & CLEANUP** -- Once green, refactor for SOLID / DRY, modularity, and high performance. Ensure code adheres strictly to language formatting standards (`gofmt` for Go, `clang-format` for C, no trivial empty lines with spaces/tabs), applying formatting only to changed/new lines without reformatting unchanged code. Actively identify and delete deprecated files, redundant logic, or replaced features. Run `ReadLints` or specific automated checking tools for code quality and security checks. Re-run tests; must stay green.
