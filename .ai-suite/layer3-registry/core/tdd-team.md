...
- ❌ Do not delete a failing test to "make it pass".
- ❌ Do not run destructive commands on remote/production hosts without confirmation.
- ❌ **Do not hardcode corpus sizes in test assertions.** Any assertion on a count that grows with the codebase (number of skills, rules, check passes, files in a directory) MUST be computed dynamically at test runtime — never as a literal integer.
