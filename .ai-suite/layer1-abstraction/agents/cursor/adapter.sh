...
_remove_registry_rules() {
  ...
  if [[ -n "$suite_dir" && -d "$suite_dir/layer3-registry/domains" ]]; then
    for domain in "$suite_dir"/layer3-registry/domains/*; do
      [[ -d "$domain/rules" ]] || continue
      for r in "$domain/rules"/*; do
        [[ -f "$r" ]] || continue
        rm -f "$rules_dir/$(basename "$r")" 2>/dev/null || true
      done
    done
  fi
  rm -f "$rules_dir"/agent-directives.md \
        "$rules_dir"/code-quality.md \
        "$rules_dir"/interactive-workflow.md \
        "$rules_dir"/nuclear-safety.md \
        "$rules_dir"/production-safety.md \
        "$rules_dir"/step-action-visibility.md 2>/dev/null || true
}
...
"Improve the suite", or "Run Reflection", follow the Reflection Protocol at:
...
agent_uninstall_project() {
  ...
  _remove_cursorrules_block "$cursorrules"
  _remove_registry_rules "$rules_dir" "$suite_dir"
...
agent_uninstall_global() {
  ...
  _remove_registry_rules "$HOME/.cursor/rules" "$suite_dir"
