...
- **Clean Empty Lines**: Empty lines must be completely clean (no spaces or tabs on blank lines / no trailing whitespace). All spaces/tabs on lines containing no real code MUST be removed to avoid gitreview and code review red flags, leaving only pure empty lines for readability.
...
- Strict prohibition against leaving trailing whitespace or spaces/tabs on empty lines, explicitly preventing gitreview red flags.
- Clear instruction to remove all spaces and tabs from lines without real code, maintaining pure empty lines for readability.
...
- MUST enforce no space/tab-only empty lines and explicit avoidance of gitreview red flags by stripping spaces/tabs on empty lines.
