...
usage() {
  cat <<'EOF'
ai_suite_workflow.sh — Streamlined workflow orchestration for ai-suite

USAGE
  ai_suite_workflow.sh <command> [options]
...
COMMANDS
  ...
  enable      Enable the AI suite for the agent
              Delegates to enable_suite.sh (supports its options)

  disable     Disable the AI suite for the agent
              Delegates to disable_suite.sh (supports its options)

  publish     Publish the AI suite as a tar package
              Delegates to publish_suite.sh (supports its options)
...
do_evolve() {
  ...
  info "After reflection completes, run ./evolve_suite.sh collect (if from remote)"
...
do_enable() {
  ...
  exec ./enable_suite.sh "$@"
}

do_disable() {
  info "Workflow: Disable"
  exec ./disable_suite.sh "$@"
}

do_publish() {
  info "Workflow: Publish"
  exec ./publish_suite.sh "$@"
}
...
do_develop() {
  ...
  printf "     ./evolve_suite.sh collect --local\n"
