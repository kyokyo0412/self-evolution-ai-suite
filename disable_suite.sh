#!/usr/bin/env bash
# disable_suite.sh — Cleanly deactivate the .ai-suite/ multi-agent framework.
#
# Symmetric to enable_suite.sh.
#
#   --scope project [--project PATH] [--agent cursor|claude|opencode|continue|roo-code|all]
#       Remove the activation block / CLAUDE.md block from the project.
#
#   --scope global [--agent cursor|claude|opencode|continue|roo-code|all]
#       Remove user-global install.
#
#   --scope remote --host USER@HOST [--remote-path PATH] [--remote-scope SCOPE]
#       ssh into the remote and run disable_suite.sh there.
  ...
  case "$REMOTE_SCOPE" in
    project)
      if [[ -n "$PROJECT_PATH" ]]; then
        ...
        remote_cmd="bash \"$REMOTE_PATH/disable_suite.sh\" --scope project --project $q_proj --agent $AGENT"
      else
        remote_cmd="bash \"$REMOTE_PATH/disable_suite.sh\" --scope project --agent $AGENT"
      fi
      ;;
    global)
      remote_cmd="bash \"$REMOTE_PATH/disable_suite.sh\" --scope global --agent $AGENT"
      ;;
