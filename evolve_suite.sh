#!/usr/bin/env bash
# evolve_suite.sh — Collect remote ai-suite evolutions into the local git
...
#
# Usage:
#   evolve_suite.sh collect --host USER@HOST [--host USER@HOST2 ...]
#                           [--remote-path PATH] [--dry-run]
#
#   evolve_suite.sh push    --host USER@HOST [--host USER@HOST2 ...]
#                           [--remote-path PATH]
...
usage() {
  cat <<'EOF'
evolve_suite.sh — sync ai-suite evolutions between remote hosts and local git

USAGE
  evolve_suite.sh collect [--local] [--host USER@HOST ...] [--remote-path PATH] [--dry-run]
  evolve_suite.sh push    --host USER@HOST [--host ...] [--remote-path PATH]
...
SUB-COMMANDS
  ...
  push      Rsync the local (evolved) .ai-suite/ to remote host(s), then
            re-run enable_suite.sh --scope <remote-scope> on each host.
...
EXAMPLES
  ...
  evolve_suite.sh collect --host alice@dev.example.com

  # Push evolved suite to multiple remotes
  evolve_suite.sh push --host alice@dev.example.com --host bob@ci.example.com
EOF
}
...
if [[ ${#HOSTS[@]} -eq 0 && "$COLLECT_LOCAL" == "0" ]]; then
  ...
  printf 'Run: evolve_suite.sh --help\n' >&2
  exit 1
fi
...
REPORT
  ...
  # Run enable_suite.sh to update the Augmented Agent
  local enabler="$SUITE_ROOT/enable_suite.sh"
  if [[ -x "$enabler" ]]; then
    info "Updating the local Augmented Agent with collected evolution ..."
    bash "$enabler" --scope project >/dev/null 2>&1 || warn "Failed to update the Augmented Agent."
  fi
...
'
  ...
  for HOST in "${HOSTS[@]:-}"; do
    ...
    local enable_cmd=(
      ...
      "$HOST"
      "bash ${remote_dir}/enable_suite.sh --scope ${REMOTE_SCOPE} --agent ${AGENT}"
    )
    local rsync_scripts=(
      rsync -az
      -e "ssh -o BatchMode=yes -o ConnectTimeout=10"
      "$SUITE_ROOT/enable_suite.sh"
      "$SUITE_ROOT/disable_suite.sh"
      "$SUITE_ROOT/evolve_suite.sh"
      "${HOST}:${remote_dir}/"
    )
    ...
    # Re-run enable_suite.sh on remote
    if ! "${enable_cmd[@]}"; then
      warn "enable_suite.sh on $HOST failed"
      failed_hosts+=("$HOST")
      continue
    fi
